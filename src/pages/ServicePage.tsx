...
function ServiceView({ service }: { service: ServiceDetail }) {
  ...
  const relatedProjects = service.relatedProjects
    .map((slug) => ({
      slug,
      detail: getProjectDetail(lang, slug),
      project: t.projects.items.find((p: any) => p.slug === slug),
    }))
    .filter((x) => x.detail && x.project) as Array<{
      slug: string
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {relatedProjects.map(({ slug, detail, project }) => (
                    <Link
                      key={slug}
                      to={`/projects/${slug}`}
                      onClick={() => goal('project_card_click', { source: 'service-page', slug })}
                    ...
                    >
                      ...
                      <div className="p-4">
                        <p className="font-head text-[9px] font-bold uppercase tracking-widest text-muted-foreground mb-1">
                          {detail.category}
                        </p>
