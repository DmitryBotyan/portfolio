...
const en: Record<string, ProjectDetail> = {
  '19ib': {
    ...
    role: ['Architecture', 'Frontend', 'Backend', 'DevOps', 'CI/CD'],
    intro: [
      '19 Independent Battle is the most iconic annual rap battle in the Russian-language hip-hop scene, run on Hip-Hop.ru. The platform handles the full lifecycle of a tournament season: round creation, track submission, anonymous judging, bracket generation, leaderboards, likes and a growing open prize pool.',
    ],
    sections: [
      {
        ...
        list: [
          ...
          'Rich-text blog (TipTap), avatar cropping, Excel export',
          'Open prize pool growing through the season',
        ],
      },
      {
        heading: 'Frontend stack',
        paragraphs: [
          'React 18 + TypeScript + Vite 6. UI built on Radix UI primitives with a shadcn/ui approach and Tailwind CSS v4. Motion for animations, Recharts for stats, TipTap for the blog editor, DOMPurify for HTML sanitization, Sonner for toasts.',
        ],
      },
      {
        heading: 'Backend stack',
        paragraphs: [
          'Bun runtime + Fastify 5 + TypeScript. PostgreSQL with Drizzle ORM for type-safe migrations and queries. JWT auth (jose), VK ID OAuth, reCAPTCHA v3. File uploads via @fastify/multipart to S3-compatible storage (Timeweb). music-metadata for track duration. Email verification via Nodemailer/AWS SES. node-cron for scheduled tasks, Zod for validation.',
        ],
      },
      {
  ...
  'ramki-bot': {
    slug: 'ramki-bot',
    metaTitle: 'RAMKI Magazine Telegram Bot: RAMKI 500 Project and Media Resources',
    metaDescription:
      'Telegram bot for the RAMKI federal business magazine. Application flow for the RAMKI 500 project uniting 500 entrepreneurs, magazine media resources and subscriber materials.',
    keywords: 'ramki magazine bot, telegram bot for magazine, ramki 500 project, business magazine bot, aiogram telegram bot, python telegram bot',
    h1: 'Telegram bot for RAMKI business magazine',
    tagline: 'Application flow for RAMKI 500, media resources and subscriber gifts in one bot',
    category: 'Telegram bot',
    year: '2025',
    client: 'RAMKI Magazine',
    industry: 'Business media',
    role: ['Bot architecture', 'Python / aiogram development', 'Content flow design', 'VPS deployment'],
    intro: [
      'RAMKI is a federal business magazine covering entrepreneurs, executives and innovators. The bot serves as the primary entry point for the magazine\'s digital funnel: it routes readers to the RAMKI 500 project, the media catalog and subscriber rewards.',
    ],
    sections: [
      {
        heading: 'What the bot does',
        paragraphs: [
          'From the very first /start the user sees a clean menu with three paths: a secret gift for new subscribers, information about the RAMKI 500 project and access to media resources. Each path is a conversation flow, not a wall of text.',
        ],
        list: [
          'Onboarding flow with a subscriber gift on /start',
          'RAMKI 500 application flow: project info, author list, participation request',
          'Media resources section: links to issues, social channels, editorial contacts',
          'PostgreSQL storage for subscriber data and application tracking',
          'Admin notifications for new applications',
        ],
      },
      {
        heading: 'RAMKI 500 project',
        paragraphs: [
          'RAMKI 500 is a large-scale publication bringing together 500 leading entrepreneurs, experts, top managers and public figures across five themed journals of 100 people each. Total print run: 5 000 copies. Cumulative target audience: 15 million. The bot handles the entire application and pre-screening flow for participation.',
        ],
      },
    ],
    faq: [
      { q: 'What stack does the bot use?', a: 'Python with the aiogram framework, PostgreSQL for data storage, Redis for session state. Deployed on a VPS as a systemd service with automatic restarts.' },
      { q: 'How are applications processed?', a: 'When a user submits an application through the bot, the data is saved to PostgreSQL and the editorial team receives an instant notification in a separate admin Telegram group.' },
    ],
  },
}
...
const ru: Record<string, ProjectDetail> = {
  '19ib': {
    ...
    sections: [
      {
        ...
        list: [
          ...
          'Rich-text блог (TipTap), кроп аватаров, экспорт в Excel',
          'Открытый призовой фонд, растущий в течение сезона',
        ],
      },
      {
        heading: 'Фронтенд',
        paragraphs: [
          'React 18 + TypeScript + Vite 6. UI на Radix UI с подходом shadcn/ui и Tailwind CSS v4. Motion для анимаций, Recharts для графиков и статистики, TipTap как rich-text редактор блога, DOMPurify для санитизации HTML, Sonner для тостов.',
        ],
      },
      {
        heading: 'Бэкенд',
        paragraphs: [
          'Bun как рантайм + Fastify 5 + TypeScript. PostgreSQL с Drizzle ORM для типобезопасных миграций и запросов. JWT-аутентификация (jose), VK ID OAuth, reCAPTCHA v3. Загрузка файлов через @fastify/multipart в S3-совместимое хранилище Timeweb. music-metadata для определения длительности треков. Email-верификация через Nodemailer/AWS SES, node-cron, Zod для валидации.',
        ],
      },
      {
    ...
    faq: [
      ...
      },
      {
        ...
        a: 'Без имён и аватаров судья оценивает трек сам по себе, а не репутацию артиста. Это снижает предвзятость и делает конкурс честнее, особенно для малоизвестных участников.',
      },
      {
        q: 'Как работает стриминг аудио?',
        a: 'Треки хранятся в S3-совместимом хранилище и отдаются через CDN с HTTP range requests. Кастомный плеер подгружает только нужный фрагмент, что существенно сокращает время начала воспроизведения при медленном соединении.',
      },
      {
  ...
  'ramki-bot': {
    slug: 'ramki-bot',
    metaTitle: 'Telegram-бот журнала «РАМКИ»: проект «РАМКИ 500» и медиаресурсы | Кейс',
    metaDescription:
      'Telegram-бот для федерального бизнес-журнала «РАМКИ». Заявки на участие в проекте «РАМКИ 500», медиаресурсы журнала и материалы для подписчиков.',
    keywords: 'бот журнала рамки, telegram бот для журнала, рамки 500 проект, бот бизнес-журнала, aiogram telegram бот, python telegram бот',
    h1: 'Telegram-бот бизнес-журнала «РАМКИ»',
    tagline: 'Заявки на «РАМКИ 500», медиаресурсы и подарки подписчикам в одном боте',
    category: 'Telegram-бот',
    year: '2025',
    client: 'Журнал «РАМКИ»',
    industry: 'Деловые медиа',
    role: ['Архитектура бота', 'Разработка на Python / aiogram', 'Проектирование контент-потоков', 'Деплой на VPS'],
    intro: [
      'РАМКИ - федеральный бизнес-журнал о предпринимателях, руководителях и новаторах. Бот работает как основная точка входа в цифровую воронку журнала: маршрутизирует читателей на проект «РАМКИ 500», медиакаталог и подарки для подписчиков.',
    ],
    sections: [
      {
        heading: 'Что умеет бот',
        paragraphs: [
          'С первого /start пользователь видит чистое меню из трёх путей: секретный подарок для новых подписчиков, информация о проекте «РАМКИ 500» и доступ к медиаресурсам. Каждый путь - это диалоговый сценарий, а не стена текста.',
        ],
        list: [
          'Онбординговый поток с подарком подписчику при /start',
          'Поток заявки на «РАМКИ 500»: информация о проекте, список авторов, форма участия',
          'Раздел медиаресурсов: ссылки на выпуски, соцсети, редакционные контакты',
          'PostgreSQL для хранения данных подписчиков и отслеживания заявок',
          'Уведомления для администраторов о новых заявках',
        ],
      },
      {
        heading: 'Проект «РАМКИ 500»',
        paragraphs: [
          '«РАМКИ 500» - масштабный проект, объединяющий 500 сильнейших предпринимателей, экспертов, топ-менеджеров и выдающихся людей в пяти тематических журналах по 100 человек. Совокупный тираж: 5 000 экземпляров. Суммарная целевая аудитория: 15 млн. Бот закрывает весь поток заявок и предварительного отбора участников.',
        ],
      },
    ],
    faq: [
      { q: 'Какой стек использует бот?', a: 'Python с фреймворком aiogram, PostgreSQL для хранения данных, Redis для состояния сессий. Задеплоен на VPS как systemd-сервис с автоматическим перезапуском.' },
      { q: 'Как обрабатываются заявки?', a: 'Когда пользователь отправляет заявку через бота, данные сохраняются в PostgreSQL и редакция сразу получает уведомление в отдельную admin-группу в Telegram.' },
    ],
  },
}
